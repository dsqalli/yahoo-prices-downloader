...
class Downloader():
    ...
    def __init__(
        ...
        """
        adj_close_only is set on True by default. 
        Explicitly set it to False if you want the complete yahoo finance historical price data.
    ...
    def save_to_csv(self,path='data/'):
        ...
        try:
            csv_file_name = ("/" + self.ticker + "_from_{}_to_{}.csv".format(
                                                                      self.start_date, 
                                                                      self.end_date))
